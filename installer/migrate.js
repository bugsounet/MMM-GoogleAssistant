...
copyCredentials = function() {
  return new Promise((resolve, reject) => {
    ...
    fs.copyFile(GatewayCredentials, GACredentials , fs.constants.COPYFILE_EXCL, 
      err => {
        if (err) reject(err)
        else {
...
copyTokens = function() {
  return new Promise((resolve, reject) => {
    ...
    fs.cp(GatewayTokens, GATokens, {recursive: true},
     (err) => {
       if (err) reject(err)
        else {
...
launchCheckGateway = function () {
  checkGateway()
    .then(result => launchCheckCredentials())
    .catch (error => catchError(error))
}

launchCheckCredentials = function () {
  checkCredentials()
    .then(result => launchCopyCredentials())
    .catch (error => catchError(error))
}

launchCopyCredentials = function () {
  copyCredentials()
    .then(result => launchCopyTokens())
    .catch (error => catchError(error))
}

launchCopyTokens = function() {
  copyTokens()
    .then(result => launchCheckConfig())
    .catch (error => catchError(error))
}

launchCheckConfig = function() {
  checkConfig()
